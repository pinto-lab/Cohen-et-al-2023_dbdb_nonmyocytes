...
condition_1 = "dbdb"  
condition_2 = "dbhplus"
regulation = paste0("UR_in_", condition_1)  # UR or DR in condition_1
...
UR.gene.list<-unique(top10$gene)


genes.plot = sort(as.character(UR.gene.list),decreasing = TRUE)
...
# obtain average gene expression of genes in UR.gene.list in each cell population
top<-NULL
for(i in 1:length(names(table(pops)))){ 
  DE.results <- read_xlsx(path_de_datasheet, sheet=names(table(pops))[i])
  DE.results <- DE.results[DE.results[[pct_nonzero_cond_1]]>= pct_nonzero|DE.results[[pct_nonzero_cond_2]]>= pct_nonzero,]
  UR_or_DR.genes <- DE.results[DE.results$gene%in%UR.gene.list,]
  UR_or_DR.genes$cell_type <- names(table(pops))[i]
  UR_or_DR.genes$FC <- (UR_or_DR.genes[[avg_expression_cond_1]]/UR_or_DR.genes[[avg_expression_cond_2]])
  UR_or_DR.genes$FC[UR_or_DR.genes$FC>6] <- 6
...
p <- p + geom_point(data=data.to.plot.new[data.to.plot.new$sig=="1",], aes(x=gene,y=cell_type),colour="black",size=0.4)
